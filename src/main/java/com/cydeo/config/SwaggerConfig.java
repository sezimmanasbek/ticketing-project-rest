...
public class SwaggerConfig {
    ...
    public OpenAPI customOpenApi(){
        return new OpenAPI().info(getInfo()).components(new Components().addSecuritySchemes(OAUTH_SCHEME_NAME,createOAuthScheme()))
                .addSecurityItem(new SecurityRequirement().addList(OAUTH_SCHEME_NAME));
    }

    private Info getInfo(){
        return new Info().title("Ticketing App Rest").description("Api documentation").version("v1.0");
    }
    ...
    private OAuthFlow createAuthorizationCodeFlow() {
        ...
        var protocolUrl = String.format(PROTOCOL_URL_FORMAT,keycloakProperties.getAuthServiceUrl(),keycloakProperties.getRealm());
        return new OAuthFlow().authorizationUrl(protocolUrl+"/auth")
